...
sed -i '/proxy_set_header X-Forwarded-Proto $scheme;/c\      #proxy_set_header X-Forwarded-Proto $scheme;' ./common/config/nginx/nginx.conf
sed -i 's/realm: http:\/\//realm: '${HARBOR_PROTOCOL}':\/\//g' ./common/config/registry/config.yml
